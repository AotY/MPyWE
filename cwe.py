    ...
    while fi.tell() < end:  #
        ...
        for sent_pos, token in enumerate(sent):
            ...
            if cbow:
                ...
                for c in context:
                    neu1c = np.zeros(dim)
                    neu1c = np.add(neu1c, syn0[c])

                    # 加上 character
                    if len(vocab[c].character) > 0:
                        for character_index in vocab[c].character:
                            neu1c = np.add(neu1c, np.multiply(syn0_c[character_index], 1.0 / len(vocab[c].character)))
                            character_index_list.append(character_index)

                        neu1c = np.multiply(neu1c, 1.0 / 2)

                    neu1 = np.add(neu1, neu1c)
            ...
            # Skip-gram
                ...
                for c in context:
                    ...
                    for target, label in classifiers:
                        ...
                        if len(vocab[c].character) > 0:
                            for character_index in vocab[c].character:
                                neu1c += np.multiply(syn0_c[character_index], 1.0 / len(vocab[c].character))
...
def __init_process(*args):
    ...
    vocab, syn0_tmp, syn0_pinyin_tmp, syn1_tmp, table, cbow, neg, dim, starting_alpha, win, num_processes, character_rate, global_word_count = args[
                                                                                                                                               :-1]
    ...
    with warnings.catch_warnings():
        ...
        syn0_c = np.ctypeslib.as_array(syn0_pinyin_tmp)
...
def train(fi, fo, cbow, neg, dim, alpha, win, min_count, num_processes, binary, character_rate):
    ...
    # Init net
    syn0, syn0_c, syn1 = init_net(dim, len(vocab), (MAX_CHINESE - MIN_CHINESE + 1))
...
if __name__ == '__main__':
    ...
    output_file = "/Users/LeonTao/PycharmProjects/deborausujono/word2vecpy/data/people's_daily_character_cbow_100d"
