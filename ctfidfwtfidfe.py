...
adding tf-idf weight of character

adding tf-idf weight of word
...
idf_characters_dict = pickle.load(open("./data/people's_daily_idf_characters_dict.pkl", "rb"))

tf_article_character_counter_dict = pickle.load(open("./data/people's_daily_tf_article_character_counter_dict.pkl", "rb"))
...
def train_process(pid):
    ...
    while fi.tell() < end:  #
        ...
        if line.startswith(u'######'):
            ...
            if tf_article_character_counter_dict is None or len(tf_article_character_counter) == 0:
                continue
            ...
            # 排序
            tf_article_character_counter = sorted(tf_article_character_counter.items(), key=lambda x: x[1], reverse=True)
            max_count = tf_article_character_counter[0][1]
            for c, count in tf_article_character_counter:
                print(u'c: {}, count: {}'.format(c, count))
                tf = count / max_count
            ...
            if cbow:
                ...
                for c, word_weight in zip(context, norm_words_weight):
                    ...
                    if len(vocab[c].character) > 0:
                        ...
                        characters_tf_idfs = np.array([article_characters_tf_idf[character_index] for character_index in vocab[c].character])
                ...
                # neu1 = np.multiply(neu1, 1.0 / len(context)) # 这里又是取平均，不应该这样吧 ？
...
if __name__ == '__main__':
    ...
    # output_file = "/Users/LeonTao/PycharmProjects/deborausujono/word2vecpy/data/people's_daily_morpheme_cbow_100d"
