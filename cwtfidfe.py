        ...
        for sent_pos, token in enumerate(sent):
            ...
            if cbow:
                ...
                for c, word_weight in zip(context, norm_words_weight):
                    print('word_index: {}, word_weight: {}'.format(c, word_weight))
                    neu1c = np.zeros(dim)
                    ...
                    if len(vocab[c].character) > 0:
                        ...
                        for character_index in vocab[c].character:
                            neu1c += syn0_c[character_index] * 1.0 / len(vocab[c].character)
            ...
            # Skip-gram
                ...
                for c, word_weight in zip(context, norm_words_weight):
                    ...
                    if len(vocab[c].character) > 0:
                        # 获取 character 的 tf-idf ，标准化
                        for character_index in vocab[c].character:
                            weight = 1.0 / len(vocab[c].character)
...
def __init_process(*args):
    ...
    vocab, syn0_tmp, syn0_c_tmp, syn1_tmp, table, cbow, neg, dim, starting_alpha, win, num_processes, character_rate, global_word_count = args[
                                                                                                                                              :-1]
...
if __name__ == '__main__':
    ...
    output_file = "/Users/LeonTao/PycharmProjects/deborausujono/word2vecpy/data/people's_daily_character_word_tfidf_cbow_100d_2"
