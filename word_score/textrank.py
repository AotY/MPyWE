...
class TextRank(object):
    ...
    def top_index(self, limit=None):
        if limit is None:
            return list(map(lambda x: x[0], self.top))[:]
        else:
...
class KeywordTextRank(object):
    ...
    # def top_index(self, limit=None):
    #     if limit is None:
    #         return list(map(lambda x: (x[0], x[1]), self.top))[:]
    #     else:
    #         return list(map(lambda x: (x[0], x[1]), self.top))[:limit]
    #
