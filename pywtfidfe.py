...
# pinyin_hash_list = [-1] * pinyin_size
    ...
    def __init__(self, word):
        ...
        if is_all_chinese:
            ...
            for pl in pinyins:
                for p in pl:
                    h = hash_pinyin(p)
                    # while pinyin_hash_list[h] != -1:
                    #     h = (h + 1) % pinyin_size

                    # pinyin_hash_list[h] = p
...
class Vocab:
    ...
    def encode_huffman(self):
        ...
        for i in range(vocab_size - 1):
            ...
            binary[min2] = 1  # 有点像桶标记思路，默认每个元素为0，右孩子节点为1
        ...
        for sent_pos, token in enumerate(sent):
            ...
            if cbow:
                ...
                for target, label in classifiers:
                    ...
                    neu1e += np.multiply(syn1[target], g)  # Error to backpropagate to syn0

                    syn1[target] += np.multiply(neu1, g)  # Update syn1
