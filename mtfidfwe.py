...
adding tf-idf weight of morpheme
'''
...
words_idf = pickle.load(open("./people's_daily_words_idf.pkl", "rb"))

article_word_counter_dict = pickle.load(open("./people's_daily_article_word_counter_dict.pkl", "rb"))
        ...
        if is_all_chinese:
            if len(word) == 1:
                ...
                if word in disyllable_bf:
                    ...
                    for c in word:
                        if c in monosyllable_bf:
                            self.morpheme.append(c)
            else:
                if word in multisyllable_bf:
                    ...
                    for i in range(len(word) - 1):
                        if tmp_word[i: i + 1 + 1] in disyllable_bf:
                            self.morpheme.append(tmp_word[i: i + 1 + 1])
                            tmp_list.append(tmp_word[i: i + i + 1 + 1])
                    for i in tmp_list:
                        tmp_word.replace(i, '')
                    for c in tmp_word:
                        if c in monosyllable_bf:
...
def train_process(pid):
    ...
    while fi.tell() < end:  #
        ...
        if line.startswith('#####'):
            ...
            article_word_counter = article_word_counter_dict.get(file_name)

            # 计算每一篇文章中每一个字（语素）的tf-idf
            morphemes_tf_idf = {}
            # 排序
            sorted(article_word_counter.items(), key=lambda x: x[1], reverse=True)
            max_count = article_word_counter.values()[0]
            for morpheme, count in article_word_counter.iteritems():
                tf = count / max_count
                tf_idf = tf * words_idf[morpheme]
                morpheme_index = hash_morpheme(morpheme)
                morphemes_tf_idf[morpheme_index] = tf_idf
        ...
        for sent_pos, token in enumerate(sent):
            ...
            if cbow:
                ...
                morpheme_index_list = []
                morpheme_weight_list = []

                for c in context:
                    neu1m = np.zeros(dim)

                    neu1m = np.add(neu1m, syn0[c, :])

                    if len(vocab[c].morpheme) > 0:
                        # 获取 morpheme 的 tf-idf ，标准化
                        # 加上 morpheme
                        morphemes_tf_idfs = np.array([morphemes_tf_idf[morpheme_index] for morpheme_index in vocab[c].morpheme])
                        norm_morphemes_tf_idfs = morphemes_tf_idfs / np.sum(morphemes_tf_idfs)
                        for morpheme_index, weight in zip(vocab[c].morpheme, norm_morphemes_tf_idfs):
                            # neu1m = np.add(neu1m, np.multiply(syn0_m[morpheme_index, :], 1.0 / len(vocab[c].morpheme)))

                            neu1m += syn0_m[morpheme_index] * weight
                            morpheme_index_list.append(morpheme_index)
                            morpheme_weight_list.append(weight)
                ...
                for morpheme_index, weight in zip(morpheme_index_list, morpheme_weight_list):
                    # syn0_m[morpheme_index] += neu1e * morpheme_rate
                    syn0_m[morpheme_index] += neu1e * weight
...
if __name__ == '__main__':
    ...
    output_file = "/Users/LeonTao/PycharmProjects/deborausujono/word2vecpy/data/people's_daily_morpheme_tfidf_cbow_100d"
